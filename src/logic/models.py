...
from src.logic.validators import validate_game_state, validate_grid
from src.logic.exceptions import InvalidMove, UnknownGameScore
        ...
        for pattern in WINNING_PATTERNS:
            for mark in Mark:
                if re.match(pattern.replace("?", mark), self.grid.cells):
                    return [
                        match.start()
                        for match in re.finditer(r"\?", pattern) # find position of `?` in pattern
                    ]
        return []
